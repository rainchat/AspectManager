    ...
    public static void sendTranslation(CommandSender p, boolean prefixed, String text, PlaceholderSupply<?>... replacementSource) {
        ...
        if (player != null) {
            ...
            if (!titleMessage.isEmpty() || !subtitleMessage.isEmpty()) {
                titleMessage = CubeCore.getAPI().getPlaceholderManager().getReplacements(player, titleMessage);
                subtitleMessage = CubeCore.getAPI().getPlaceholderManager().getReplacements(player, subtitleMessage);
                sendTitle(player, titleMessage, subtitleMessage, 10, 70, 20);
            }

            if (!actionbarMessage.isEmpty()) {
                actionbarMessage = CubeCore.getAPI().getPlaceholderManager().getReplacements(player, actionbarMessage);
